...
class ProjectModel extends AdminModel
{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get item id
		$id = (int) $this->getState('project.id', $app->getInput()->get('id', 0));

		// Modify the form based on Edit State access controls
		if ($id != 0 && !$app->getIdentity()->authorise('core.edit.state', 'com_swjprojects.project.' . $id))
		{
	...
	public function getTranslateForms($loadData = true, $clear = false)
	{
		...
		$file  = JPATH_COMPONENT . '/forms/translate_project.xml';
		if (!is_file($file))
		{
	...
	{
		$data = Factory::getApplication()->getUserState('com_swjprojects.edit.project.data', []);
		if (empty($data))
		{
			$data = $this->getItem();
		}
	...
	public function save($data)
	{
		...
		{
			$additional_categories         = (!empty($data['additional_categories'])) ? $data['additional_categories'] : [];
	 ...
	 * @since  1.0.0
	 */
	public function getTable($name = 'Projects', $prefix = 'Administrator', $options = [])
	{
