...
class VersionsModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query = $db->getQuery(true)
			->select(['v.*', 'CONCAT(v.major, ".", v.minor, ".", v.patch) as version'])
			->from($db->quoteName('#__swjprojects_versions', 'v'));

		// Join over the projects
		$query->select(['p.id as project_id', 'p.element as project_element'])
		...
		if (!empty($search))
		{
			...
			{
				$sql     = [];
				$columns = ['v.tag', 'p.element', 't_p.title', 'ta_v.changelog'];
