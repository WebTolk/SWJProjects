...
final class Joomlaserverscheme extends ServerschemePlugin implements SubscriberInterface
{
	...
	protected function buildProjectUpdatesXml(array $data): string
	{
		...
		foreach ($data as $item) {
			...
			$this->appendLibraryMaintainerData($update, $item);
	...
	/**
	 * Add maintainer metadata for library update manifests.
	 *
	 * Joomla's updater understands maintainer fields in update XML, while
	 * SW JProjects keeps the generic maintainer payload in the model layer.
	 *
	 * @param   SimpleXMLElement  $update  The update XML node.
	 * @param   object            $item    Version item payload.
	 *
	 * @return  void
	 *
	 * @since   2.6.2
	 */
	private function appendLibraryMaintainerData(SimpleXMLElement $update, object $item): void
	{
		...
		if (!is_object($maintainer)) {
			return;
		}

		$title = property_exists($maintainer, 'title') ? trim((string) $maintainer->title) : '';
		$url = property_exists($maintainer, 'website') ? trim((string) $maintainer->website) : '';

		if ($title !== '') {
			$update->addChild('maintainer', $title);
		}

		if ($url !== '') {
			$update->addChild('maintainerurl', $url);
		}
	}
